...
|  | `DOCLING_SERVE_MAX_TASKS_BEFORE_RESTART` | `0` (disabled) | When set to a positive number and `FREE_VRAM_ON_IDLE=True`, the server will automatically restart after processing N tasks to ensure complete VRAM cleanup. This is required for ONNX Runtime CUDA memory release (which doesn't release memory in-process). Requires `restart: unless-stopped` in docker-compose.yml. Recommended values: `1-5` for maximum VRAM efficiency, `20-50` for balanced approach, `100+` for minimal restart overhead. Set to `0` to disable. |
...
|  | `DOCLING_SERVE_UNLOAD_LLAMA_SWAP_API_KEY` | unset | Optional: API key for llama-swap authentication. When llama-swap is configured with API key protection, set this to the key value. The key is sent via the `X-Api-Key` header. |
|  | `DOCLING_SERVE_UNLOAD_EXTERNAL_MODEL_TIMEOUT` | `10.0` | Timeout in seconds for HTTP requests to external model unloading services (Ollama, llama-swap). Increase this value for systems under heavy load or with slow network connections. |
